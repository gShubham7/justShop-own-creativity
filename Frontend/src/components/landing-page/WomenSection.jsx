...
import cartblack from "../../assets/images/cartblack.png";
  ...
  return (
    <Stack className="menSec" direction={["column", "row"]} gap="3rem">
      <Box className="menBox">
        <Image
          className="menImage"
          src="https://assets.myntassets.com/h_1440,q_100,w_1080/v1/assets/images/productimage/2020/10/11/b0050a33-18a0-436c-b3a6-e05d2b303ebd1602367973649-1.jpg"
        />
        <Box style={{ fontSize: "1.6rem", fontWeight: "bold" }}>
          Women's Accessories
        </Box>
        <Link to="/women-clothing">
          <Button style={{ backgroundColor: "#0D6DD7", color: "#F3F3f3" }}>
            Show All Products
          </Button>
        </Link>
      </Box>
          ...
          {items.map((item) => (
            <Box key={item.id} className="sellerBox">
              <Box className="imageBox">
                <Image src={item.src} alt="sellers" width="170px" />
              </Box>
              <Box>
                <Text fontSize="15px" color="#333333">
                  {item.title}
                </Text>
                <Flex justifyContent="center" alignItems="center" gap="1rem">
                  <Text fontSize="15px" fontWeight="semibold" color="#0D6DD7">
                    ₹ {item.price}
                  </Text>
                  <Button>
                    <Image src={cartblack} alt="cart" width="20px" />
                  </Button>
                </Flex>
              </Box>
            </Box>
